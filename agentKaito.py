...
class Kaito1(Player):
    ...
    def __init__(self, player_num):
        ...
        self.opponent = Opponent(3-player_num)  # 相手の仮想エージェント
        self.times = 1  # 現在のゲームの回数
        self.turn = 0  # 各ゲームにおける(自分の)ターン
        self.card = None
        self.log = []  # 各セットにおけるログ
        self.won = {'Kaito': 0, 'Opponent': 0}  # 各セットにおける勝数
        ...
        self.mode = self.set_mode()  # プレイスタイルのモード
    ...
    def initialize(self):
        ...
        self.call_list = []
        if self.times % 3 == 0:
            self.opponent = Opponent(3-self.player_num)
            self.card = None
            self.log = []
            self.won = {'Kaito': 0, 'Opponent': 0}
            self.guard_items = ['shuffle']
            self.attack_items = ['high_and_low', 'slash']
        self.turn = 0
        self.times += 1
    ...
    def select_guard(self):
        if len(self.opponent.get_candidate_list()) <= 100 and len(self.guard_items) != 0:
            item = 'shuffle'
            self.guard_items.remove(item)
            return item
        else:
            return None
    ...
    def select_attack(self):
        if random.uniform(0, 1) < 0.7:
            if len(self.attack_items) != 0:
                item = random.choice(self.attack_items)
                self.attack_items.remove(item)
                return item
            else:
                return None
        else:
    ...
    def shuffle(self):
        new_card = random.choice([''.join(n) for n in itertools.permutations(self.card)])
        self.card = new_card
        return new_card
    ...
    def end_process(self, winner, cards_record):
        self.initialize()
        self.opponent.end_process(winner, cards_record)
        self.mode = self.set_mode()

    def set_mode(self):
        ''' プレイモードの指定 '''
        if self.won['Kaito'] == self.won['Opponent'] == 0:
            mode = 'normal'
        elif self.won['Kaito'] == 1 and self.won['Opponent'] == 0:
            mode = 'normal'
        elif self.won['Kaito'] == 0 and self.won['Opponent'] == 1:
            mode = 'normal'
        elif self.won['Kaito'] == 1 and self.won['Opponent'] == 1:
            mode = 'full-power'
        elif self.won['Kaito'] == 2 and self.won['Opponent'] == 0:
            mode = 'normal'
        elif self.won['Kaito'] == 0 and self.won['Opponent'] == 2:
            mode = 'normal'
        return mode
